...
The first time qtools opens, a short wizard asks for the language, the colour theme and the icons, each with an "In every Quvyta application" box that shares the choice with every other Quvyta application. Its second page says what qtools promises before it changes anything, and has one box, **Say when an update is out** (see [below](#no-telemetry-and-what-goes-over-the-network)). Nothing is written until you finish; closing it half-way leaves everything as it was, and the wizard comes again next time. Finishing writes `~/.config/quvyta/tools.conf`, which holds those three choices, and the shared `~/.config/quvyta/quvyta.conf` if it is not there yet. Unchecking the box writes `update-notice = false` into that file, for every Quvyta application. Everything the wizard asks can be changed later on the Settings page.

## Settings

**Settings** is the last entry of the sidebar, below the groups, and the last tab on a narrow screen. It shows the same rows as every Quvyta application: language, theme and icons, each with its **In every Quvyta application** box, then reduced motion, the pillar and **Say when an update is out**. There is no Save button: a change takes effect at once and is written straight away. With the box checked it goes to the shared `~/.config/quvyta/quvyta.conf`, and every Quvyta application that follows it changes too; unchecked, it stays in qtools' own `~/.config/quvyta/tools.conf`. Reduced motion and the pillar are qtools' own and are kept in `tools.conf`. If a file cannot be written, the change still applies until qtools quits and its row says why it was not saved. The update switch is written in the background; if it cannot be, it goes back to what it was and a notice says which folder and why.
