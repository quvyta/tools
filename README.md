...
The first time qtools opens, a short wizard asks for the language, the colour theme and the icons, each with an "In every Quvyta application" box that shares the choice with the rest of the family. Its second page says what qtools promises before it changes anything, and has one box, **Say when an update is out** (see [below](#no-telemetry-and-what-goes-over-the-network)). Nothing is written until you finish; closing it half-way leaves everything as it was, and the wizard comes again next time. Finishing writes `~/.config/quvyta/tools.conf`, which holds those three choices, and the family's `~/.config/quvyta/quvyta.conf` if it is not there yet. Unchecking the box writes `update-notice = false` into that file, for the whole family. Everything the wizard asks can be changed later on the Settings page.

## Settings

**Settings** is the last entry of the sidebar, below the groups, and the last tab on a narrow screen. It shows the same rows as every Quvyta application: language, theme and icons, each with its **In every Quvyta application** box, then reduced motion, the pillar and **Say when an update is out**. There is no Save button: a change takes effect at once and is written straight away. With the box checked it goes to the family's `~/.config/quvyta/quvyta.conf`, and every Quvyta application that follows the family changes too; unchecked, it stays in qtools' own `~/.config/quvyta/tools.conf`. Reduced motion and the pillar are qtools' own and are kept in `tools.conf`. If a file cannot be written, the change still applies until qtools quits and its row says why it was not saved. The update switch is written in the background; if it cannot be, it goes back to what it was and a notice says which folder and why.
...
It asks one question of its own accord: whether a newer qtools is out. When qtools opens, at most once a day, it reads the list of published versions of `quvyta-tools` from crates.io, the same file `cargo install` reads: one HTTPS `GET` of `https://index.crates.io/qu/vy/quvyta-tools`. The request carries no cookie and no identifier; its headers are `User-Agent: quvyta-tools/<the version you run>`, `Accept: */*` and `Accept-Encoding: gzip`. crates.io sees, as with any connection, the address it comes from. The question never holds up the screen. When a newer version is out, a notice says which one and how to update; if an item is running at that moment, the notice waits until you close its terminal. When there is no network, or crates.io does not answer within ten seconds, nothing is said and the next day asks again. Nothing is asked while the first-start wizard is open, nor by `qtools --run` and `qtools --revert`, nor on a distribution qtools does not support.

The time of the last question is kept in `~/.local/state/quvyta/tools/update-check` (or under `$XDG_STATE_HOME/quvyta/tools/`), the family's folder for qtools. That is a different folder from `~/.local/state/quvyta-tools/`, where qtools keeps its backups and journal; the question never reads or writes there.

To turn it off, uncheck **Say when an update is out** on the wizard's second page or, later, on qtools' [Settings](#settings) page. The switch belongs to the whole Quvyta family: it is `update-notice = false` in `~/.config/quvyta/quvyta.conf`, and turning it off stops the question in every Quvyta application. The same switch is on the settings of quvyta and of every other Quvyta application that asks, and that line can be edited by hand. While it is off, qtools asks nothing at all.
