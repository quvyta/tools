...
//!
//! Every test lives in a temporary root: the family's folder and the fonts the appearance step
...
/// qtools on a machine whose family folder is `root/config`, on a screen of `width` by
...
/// A family another member has already set up: English, the default theme, Unicode icons. The
...
/// Every file of the family's folder with what it holds, to tell "exactly as it was".
...
fn finish_writes_both_files_and_opens_the_list() {
    ...
    assert!(own.contains("language = \"quvyta\""), "the language follows the family:\n{own}");
    let screen = h.screen();
...
/// only come from the family's file.
...
#[test]
fn with_tools_conf_there_is_no_wizard_and_the_family_language_is_in_force() {
    let root = Root::new();
    let code = not_detected();
    fs::create_dir_all(root.path().join("config")).expect("folder");
    let family = format!("language = \"{code}\"\ntheme = \"monochrome\"\nicons = \"unicode\"\n");
    fs::write(root.path().join("config/quvyta.conf"), family).expect("family file");
    ...
    for key in ["group.packages", "tweak.mirrors.title", "state.off"] {
        ...
        assert!(screen.contains(&text), "`{text}`: the family's language is in force:\n{screen}");
    }
    assert_eq!(contents(root.path()), before, "starting writes nothing");
}

#[test]
fn a_family_file_missing_beside_tools_conf_is_written_for_the_next_member() {
    // Without the wizard qtools resolves the look as every member does, and the first to start
    // leaves the family's file for the others.
    let root = Root::new();
    fs::create_dir_all(root.path().join("config")).expect("folder");
    fs::write(root.path().join("config/tools.conf"), "language = \"quvyta\"\n").expect("own file");
    let h = start(root.path(), 80, 30);
...
#[test]
fn a_theme_chosen_for_the_family_is_the_theme_at_start() {
    let root = Root::new();
    fs::create_dir_all(root.path().join("config")).expect("folder");
